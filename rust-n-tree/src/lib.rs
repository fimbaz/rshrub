...
impl<P, R: Region<P>> NTree<R, P> {
    ...
    pub fn new(mut region: R, size: u8) -> NTree<R, P> {
        NTree {
            kind: Branch {
                subregions: region
                    .split()
    ...
    pub fn range_query_mut<'t, 'q>(&'t mut self, query: &'q R) -> RangeQueryMut<'t, 'q, R, P> {
        RangeQueryMut {
...
pub struct RangeQueryMut<'t,'q, R: 'q + 't, P: 't> {
    query: &'q R,
...
impl<'t, 'q, R: Region<P>, P> Iterator for RangeQueryMut<'t, 'q, R, P> {
    type Item = &'t mut P;
